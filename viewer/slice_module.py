...
class SliceData(module_template.Data):
    ...
    def get_total_max(self):
        ...
        return numpy.array([image.max() for image in self._images]).max()

    def get_total_min(self):
        """Gives an estimate of the minimum intensity in the entire dataset"""
        #this doesn't take scaling into account because it would take to much time
        return numpy.array([image.min() for image in self._images]).min()
...
class SliceGenerator(object):
    ...
    def get_slice(self, image, rotation):
        ...
        provided rotation."""
        # this_poly_data = vtk.vtkPolyData()
...
class SliceViewer(module_template.Viewer):
    ...
    def _setup_slice_view(self):
        ...
        self._lut = vtk.vtkLogLookupTable()
        self._lut.SetTableRange(max(self._data.get_total_min(), self._data.get_total_max()/1e4),
                                self._data.get_total_max())
        self._lut.SetNumberOfColors(number_of_colors)
        self._lut.Build()
        for i in range(number_of_colors):
            color = matplotlib.cm.jet(float(i)/float(number_of_colors))
            self._lut.SetTableValue(i, color[0], color[1], color[2], 1.)
    ...
    def remove_slice(self, index, render=False):
