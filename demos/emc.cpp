 ...
 */
    ...
    for (int i = 0; i < N_model; i++) {
       ...
       model_init(conf,model, weight,images,mask, x_coordinates, y_coordinates, z_coordinates);
    ...
    for (int iteration = 0; iteration < conf.number_of_iterations; iteration++) {
       ...
       i.e. the one with the highest responsability. */
        ...
        if (iteration == 0 && strcmp(conf.initial_rotations_file, "not used") ==0){
                //printf("%s %d\n\n", conf.initial_rotations_file,strcmp(conf.initial_rotations_file, "not used"));
            cuda_set_to_zero(d_best_respons,N_images);
            reset_to_zero(respons,N_images*N_slices, sizeof(real));
       ...
       containing all iterations. */
        for (int i_image = 0; i_image < N_images; i_image++) {
            printf("DEBUG.. BEST_ROT %d %d %g\n",i_image, best_rotation[i_image], best_respons[i_image]);
       ...
       file for each iteration. */
        ...
        for (int i_image = 0; i_image < N_images; i_image++) {
            printf("DEBUG.. BEST_QUAT %d %d %g %g %g %g\n", i_image, best_rotation[i_image], rotations[best_rotation[i_image]].q[0], rotations[best_rotation[i_image]].q[1],rotations[best_rotation[i_image]].q[2], rotations[best_rotation[i_image]].q[3]);
        ...
        for (int slice_start = 0; slice_start < N_slices; slice_start += slice_chunk) {
            ...
            cuda_calculate_fit(d_slices, d_images, d_mask, d_scaling,
             ...
             //the single fit at the best rotation
        ...
        for (int i_image = 0; i_image < N_images; i_image++) {
            printf("DEBUG ... FIT FIT_BEST_ROT BESTROT %g %g %d\n", fit[i_image], fit_best_rot[i_image], best_rotation[i_image]);
     ...
     is turned off. */
            if (conf.compact_output == 0) {
                printf("DEBUG... %g %g %g %g \n\n", scaling[0],scaling[100],
                       scaling[N_slices*N_images-2],scaling[N_slices*N_images-100]);
        ...
        else cuda_copy_real_to_device(scaling,d_scaling,N_slices*N_images);
       ...
       updated. */
        ...
        cuda_copy_real_to_host(respons,d_respons,N_slices*N_images);
        /*printf("DEBUG... respons1 ");
        for (int i =0; i<200; i++){ printf("%g ", respons[N_slices * N_images - 1- i]);}
        printf("\n\n ");
   */
     ...
     included and excluded diffraction patterns */
        ...
        cuda_copy_real_to_host(respons, d_respons, N_slices*N_images);
        /*printf("DEBUG... respons3 ");
        for (int i =0; i<200; i++){ printf("%g ", respons[N_slices * N_images/2 - 1- i]);}
        printf("\n\n ");
*/
