 ...
 * 2016-Nov stochastic EMC WITH/WITHOUT optimized EMC kernels,
 *  branched from opt_emc.cu
 */
...
int main(int argc, char *argv[]){
    ...
    Quaternion *all_rotations;
    real *all_weights_rotation;
    Quaternion *rotations;
    real *weights_rotation;
    int N_slices =0;
    int all_N_slices = read_rotations_file(conf.rotations_file, &all_rotations, &all_weights_rotation);
    if (conf.isSochasticEMCon == 1){
        N_slices = (int) all_N_slices*conf.rotation_sample_ratio;
        rotations = (Quaternion*)malloc(N_slices*sizeof(Quaternion));
        weights_rotation = (real*)malloc(N_slices*sizeof(real));
    }
    else{
        rotations = all_rotations;
        weights_rotation = all_weights_rotation;
        N_slices = all_N_slices;
    }
    ...
    thrust::device_vector<real> thrust_onesN2d(N_2d*N_images,1);
    thrust::device_vector<real> thrust_onesNslicechunck(N_2d*slice_chunk ,1);
    ...
    real* d_onesNslicechunck = thrust::raw_pointer_cast(&thrust_onesNslicechunck[0]);
    ...
    thrust:: device_vector<real> thrust_log_phi (N_2d*allocate_slice ,1);
    ...
    for (int iteration = start_iteration; iteration < conf.number_of_iterations; iteration++) {
        ...
        if(conf.isSochasticEMCon == 1){
            cout<<"sampled # of slices"<<N_slices<<endl;
            int acc_rot = 0;
            int i_rot = 0;
            double randnumber =0;
            Quaternion this_quaternion;
            while(acc_rot < N_slices){
                randnumber =   gsl_rng_uniform (rng);
                if(randnumber < conf.rotation_sample_ratio){
                    //copy_this rotaion and this weight_rotation to rotation and weight rotaion;
                    rotations[acc_rot].q[0] = all_rotations[i_rot].q[0];
                    rotations[acc_rot].q[1] = all_rotations[i_rot].q[1];
                    rotations[acc_rot].q[2] = all_rotations[i_rot].q[2];
                    rotations[acc_rot].q[3] = all_rotations[i_rot].q[3];
                    weights_rotation[acc_rot] = all_weights_rotation[i_rot];
                    //printf("random pick up rotations %d %d  %f %f %f %f %f %f\n", acc_rot,i_rot, randnumber,
                    //       rotations[acc_rot].q[0], rotations[acc_rot].q[1],rotations[acc_rot].q[2],rotations[acc_rot].q[3],
                    //       weights_rotation[acc_rot]);
                    acc_rot ++;
                }
                i_rot++;
                if(i_rot >=all_N_slices)  i_rot = 0;
            }
             cuda_copy_rotations_chunk(&d_rotations, &rotations[0], 0,allocate_slice);
        }
        ...
        if (conf.recover_scaling) {
            ...
            for (slice_start = slice_backup; slice_start < slice_end; slice_start += slice_chunk) {
                ...
                if(conf.isDebug == 1)
                {
                    ...
                    cuda_reset_real(d_log_phi, N_images*slice_chunk);
