 ...
 * 2018-07-13 load images randomly
 */
...
int main(int argc, char *argv[]){
    ...
    int* lst = (int*) malloc(N_images*sizeof(int));
    if (taskid == master){
        ...
        for (int i = 0; i < N_images; i++)
        {
            ...
            int a = (int) floor(u*N_images);
            a=a>=N_images? N_images-1:a;
            a = i==0?N_images-1:a; 
            lst[i] = a;
            cout <<a<<" ";
        }
        gsl_rng_free (r);
        FILE *f = create_file_descriptor(conf, "/randomlist.data", "wp");
        write_int_array(f, lst, N_images);
        close_file(f);
    }
    MPI_Bcast(lst,N_images,MPI_INT,master,MPI_COMM_WORLD);

   // exit(1);

    sp_matrix **images = read_images_by_list(conf,masks,lst);
    ...
    if(conf.isLessBinningOn){
        ...
        images = read_images_by_list(conf,masks,lst);
