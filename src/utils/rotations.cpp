...
{
  Quaternion *res = (Quaternion *)malloc(sizeof(Quaternion));
  res->q[0] = 1.0;
  res->q[1] = 0.0;
  res->q[2] = 0.0;
  res->q[3] = 0.0;
...
int generate_rotation_list(const int n, Quaternion **return_list, real **return_weights) {
  const int number_of_samples = n_to_samples(n);
  *return_list = (Quaternion *) malloc(number_of_samples/2*sizeof(Quaternion));
  *return_weights = (real*) malloc(number_of_samples/2*sizeof(real));
  Quaternion *rotation_list = (Quaternion *)malloc(120*sizeof(Quaternion));
  ...
  Quaternion *new_list = (Quaternion *)malloc(number_of_samples*sizeof(Quaternion));
  ...
  real *weights = (real*)malloc(number_of_samples*sizeof(real));
  real dist3;
  real scalar_product;
