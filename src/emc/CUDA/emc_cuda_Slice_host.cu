...
void cuda_update_true_poisson_slices(real * d_images, real * d_slices, int * d_mask,
    ...
    update_slices_true_poisson_kernel<<<nblocks,nthreads>>>(d_images, d_slices, d_mask, d_respons,
                                               d_scaling, d_active_images, N_images, slice_start, N_2d,
                                               d_slices_total_respons, d_rot,d_x_coordinates,
                                               d_y_coordinates,d_z_coordinates,d_model,
                                               sp_matrix_rows(images[0]),sp_matrix_cols(images[0]),
...
void cuda_insert_slices(real * d_images, real * d_slices, int * d_mask,
                        real * d_respons, real * d_scaling, real* d_slices_total_respons, int * d_active_images, int N_images,
                        int slice_start, int slice_chunk, int N_2d,
                        sp_3matrix * model, real * d_model,
                        real *d_x_coordinates, real *d_y_coordinates,
                        real *d_z_coordinates, real *d_rot,
    ...
    cudaError_t status = cudaGetLastError();
    if(status != cudaSuccess){
        printf("CUDA Error (cuda_insert_slices): %s\n",cudaGetErrorString(status));
    }
}
...
void cuda_update_true_poisson_slices_NN(real * d_images, real * d_slices, int * d_mask,
                        real * d_respons, real * d_scaling, int * d_active_images, int N_images,
                        int slice_start, int slice_chunk, int N_2d,
                        sp_3matrix * model, real * d_model,
                        real *d_x_coordinates, real *d_y_coordinates,
    ...
    update_slices_true_poisson_kernel<<<nblocks,nthreads>>>(d_images, d_slices, d_mask, d_respons,
            ...
            sp_3matrix_z(model));
    cudaThreadSynchronize();
    insert_slices_kernel_NN<<<nblocks,nthreads>>>(d_images, d_slices, d_mask, d_respons,
                                               d_scaling, N_images, N_2d,
                                               d_slices_total_respons, d_rot,d_x_coordinates,
                                               d_y_coordinates,d_z_coordinates,d_model, d_weight,
                                               sp_matrix_rows(images[0]),sp_matrix_cols(images[0]),
